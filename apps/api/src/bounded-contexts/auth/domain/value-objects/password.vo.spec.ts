...
import { PasswordErrorCode } from '../errors/password.error';
...
describe('Password Value Object', () => {
  ...
  describe('최소 길이 검증', () => {
    ...
    it('1자 비밀번호는 PASSWORD_TOO_SHORT 에러를 던진다', () => {
      expect(() => Password.create('a')).toThrow(
  ...
  describe('최대 길이 검증', () => {
    it('73자 비밀번호는 PASSWORD_TOO_LONG 에러를 던진다', () => {
      ...
      expect(() => Password.create(tooLongPassword)).toThrow(
        expect.objectContaining({ code: PasswordErrorCode.PASSWORD_TOO_LONG }),
      );
    });
  ...
  describe('소문자 검증', () => {
    it('소문자가 없으면 PASSWORD_MISSING_LOWERCASE 에러를 던진다', () => {
      expect(() => Password.create('AAAA1!BBBB')).toThrow(
        expect.objectContaining({
    ...
    it('대문자+숫자+특수문자만 있으면 PASSWORD_MISSING_LOWERCASE 에러를 던진다', () => {
      expect(() => Password.create('ABC123!@#$')).toThrow(
        expect.objectContaining({
          code: PasswordErrorCode.PASSWORD_MISSING_LOWERCASE,
        }),
      );
  ...
  describe('숫자 검증', () => {
    it('숫자가 없으면 PASSWORD_MISSING_NUMBER 에러를 던진다', () => {
      expect(() => Password.create('aaaa!bbbbb')).toThrow(
        expect.objectContaining({
    ...
    it('소문자+특수문자만 있으면 PASSWORD_MISSING_NUMBER 에러를 던진다', () => {
      expect(() => Password.create('abcd!@#$ef')).toThrow(
        expect.objectContaining({
          code: PasswordErrorCode.PASSWORD_MISSING_NUMBER,
        }),
      );
  ...
  describe('특수문자 검증', () => {
    it('특수문자가 없으면 PASSWORD_MISSING_SPECIAL_CHAR 에러를 던진다', () => {
      expect(() => Password.create('aaaa1bbbbb')).toThrow(
        expect.objectContaining({
    ...
    it('소문자+숫자만 있으면 PASSWORD_MISSING_SPECIAL_CHAR 에러를 던진다', () => {
      expect(() => Password.create('abcd123456')).toThrow(
        expect.objectContaining({
          code: PasswordErrorCode.PASSWORD_MISSING_SPECIAL_CHAR,
        }),
      );
  ...
  describe('다중 위반 시 첫 번째 에러만 반환', () => {
    it('9자 + 소문자 없음 → PASSWORD_TOO_SHORT만 반환', () => {
      expect(() => Password.create('AAA1!BBBB')).toThrow(
        expect.objectContaining({ code: PasswordErrorCode.PASSWORD_TOO_SHORT }),
      );
    });
    ...
    it('73자 + 소문자 없음 → PASSWORD_TOO_LONG만 반환', () => {
      ...
      expect(() => Password.create(tooLong)).toThrow(
        expect.objectContaining({ code: PasswordErrorCode.PASSWORD_TOO_LONG }),
      );
    });
    ...
    it('10자 + 숫자 없음 + 특수문자 없음 → PASSWORD_MISSING_NUMBER만 반환', () => {
      expect(() => Password.create('aaaabbbbbb')).toThrow(
        expect.objectContaining({
          code: PasswordErrorCode.PASSWORD_MISSING_NUMBER,
        }),
      );
