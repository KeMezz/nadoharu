...
- [ ] 2.1 RED: 텍스트 150자 제한, 이미지 4장 제한, `category` 선택 자유 문자열 규칙 단위 테스트를 먼저 작성한다
- [ ] 2.2 GREEN: Post Entity/VO에 입력 검증 규칙을 구현해 테스트를 통과시킨다
- [ ] 2.3 REFACTOR: 게시물 검증 에러 코드를 정리하고 GraphQL 에러 매핑 규칙과 연결한다

## 3. 게시물 CRUD 유스케이스 구현

- [ ] 3.1 RED: `createPost`, `updatePost`, `deletePost`, `post(id)` 유스케이스 단위 테스트(작성자 권한, 소프트 삭제 포함)를 작성한다
- [ ] 3.2 GREEN: CRUD 유스케이스를 구현하고 작성자 권한/인증 조건을 반영한다
- [ ] 3.3 GREEN: `deletePost`를 `deletedAt` 기반 소프트 삭제로 구현하고 단건 조회에서 삭제 게시물 제외를 보장한다

## 4. 커서 기반 타임라인 구현

- [ ] 4.1 RED: `posts` Connection 응답(`edges`, `pageInfo`)과 커서 페이지네이션 중복/누락 방지 통합 테스트를 작성한다
...
- [ ] 5.1 RED: 업로드 URL 발급, 포맷 허용(`jpeg/jpg/png/webp/heic/heif/gif`), 5MB 제한 테스트를 작성한다
- [ ] 5.2 GREEN: S3 호환 presigned URL 발급 서비스와 GraphQL 작업을 구현한다
- [ ] 5.3 GREEN: `createPost`/`updatePost`에서 허용 버킷/경로 이미지 URL만 수용하도록 검증을 구현한다

## 6. 통합 검증 및 웹 접근 흐름 반영

- [ ] 6.1 Post GraphQL 타입/리졸버(`createPost`, `updatePost`, `deletePost`, `post`, `posts`)를 연결하고 API 통합 테스트를 통과시킨다
- [ ] 6.2 웹에서 비인증 사용자의 `posts` 접근 시 로그인 안내를 표시하고 `post(id)` 단건 조회는 공개 접근으로 동작하도록 반영한다
- [ ] 6.3 전체 관련 테스트(api unit/integration, web unit/e2e)를 실행하고 문서/스키마 산출물을 최신화한다
