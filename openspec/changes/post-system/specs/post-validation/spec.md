...
시스템은 게시물 작성/수정 시 `content` 또는 `imageUrls` 중 최소 하나가 존재하는지 검증해야 한다(MUST). `content`는 trim 기준으로 공백 전용 문자열을 빈 값으로 처리해야 한다(MUST).
...
### Requirement: `tags`는 개수/길이/중복 규칙을 만족해야 한다

시스템은 `tags`를 선택적 문자열 배열로 처리해야 한다(MUST). `tags`가 제공된 경우 최대 10개까지 허용해야 하며(MUST), 각 태그는 trim 기준 1자 이상 20자 이하이어야 한다(MUST). 중복 태그(대소문자 무시)는 단일 태그로 정규화해야 한다(MUST).

#### Scenario: 유효한 태그 목록 입력

- **WHEN** 사용자가 10개 이하, 각 20자 이하의 태그 목록을 전달한다
- **THEN** 시스템은 태그를 수용한다

#### Scenario: 태그 개수 초과 입력

- **WHEN** 사용자가 11개 이상의 태그를 전달한다
- **THEN** 시스템은 검증 오류를 반환하고 요청을 거부한다

#### Scenario: 태그 길이 초과 입력

- **WHEN** 사용자가 trim 기준 21자 이상의 태그를 전달한다
- **THEN** 시스템은 검증 오류를 반환하고 요청을 거부한다

#### Scenario: 중복 태그 정규화

- **WHEN** 사용자가 `"Diary"`와 `"diary"`를 함께 전달한다
- **THEN** 시스템은 중복을 제거해 단일 태그로 정규화한다
