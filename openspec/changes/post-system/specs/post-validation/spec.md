...
시스템은 게시물 작성/수정 시 `content` 또는 `imageUrls` 중 최소 하나가 존재하는지 검증해야 한다(MUST). `content`는 trim 기준으로 공백 전용 문자열을 빈 값으로 처리해야 한다(MUST). `subtitle`은 보조 설명 필드이므로 최소 콘텐츠 판정(`content`/`imageUrls`)에 포함하지 않아야 한다(MUST).
...
#### Scenario: subtitle만 있고 본문/이미지 없음

- **WHEN** 사용자가 `subtitle`만 입력하고 `content`와 `imageUrls`를 비워서 게시물을 작성한다
- **THEN** 시스템은 검증 오류를 반환하고 요청을 거부한다
...
### Requirement: `subtitle`은 본문 하단 보조 설명용 선택 문자열이어야 한다

시스템은 `subtitle`을 선택적 문자열로 처리해야 하며(MUST), 입력 시 본문 하단 보조 설명으로 저장/표시해야 한다(MUST). `subtitle`은 배열이나 객체 형태를 허용하지 않아야 한다(MUST).

#### Scenario: subtitle 없이 게시물 작성

- **WHEN** 사용자가 `subtitle`을 생략하고 게시물을 작성한다
- **THEN** 시스템은 요청을 허용한다

#### Scenario: subtitle 문자열 입력

- **WHEN** 사용자가 단일 문자열 `subtitle`을 전달한다
- **THEN** 시스템은 subtitle을 보조 설명 필드로 저장한다

#### Scenario: subtitle 비문자열 입력

- **WHEN** 사용자가 배열/객체 형태의 `subtitle` 값을 전달한다
- **THEN** 시스템은 검증 오류를 반환하고 요청을 거부한다
