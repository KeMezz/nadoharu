...
시스템은 S3 호환 스토리지 업로드를 위한 presigned URL 발급 기능을 제공해야 한다(MUST). 발급 응답에는 업로드에 사용할 URL과 저장 대상 식별 정보(객체 키 또는 접근 URL)가 포함되어야 한다(MUST).
...
시스템은 이미지당 최대 5MB 용량 정책을 강제해야 한다(MUST). 제한을 초과한 파일은 업로드 URL 발급 또는 게시물 저장 단계에서 거부해야 한다(MUST).
