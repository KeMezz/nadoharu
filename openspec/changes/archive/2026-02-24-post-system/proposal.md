...
- **부가설명(subcontent)**: 본문 하단에 표시할 보조 텍스트 추가 (문자열 필드)
- **카테고리**: 게시물 분류 (선택적)
- **제약 조건**: 텍스트(content) 최대 150자, 부가설명(subcontent) 최대 150자, 이미지 최대 4장 (개별 5MB 이하, 클라이언트 압축 1MB 이하 권장)
...
- Prisma 모델: `Post` (content, subcontent, category, imageUrls, authorId)
...
- 비인증 사용자의 `posts` 접근 시 로그인 안내 UX 필요
- `post(id)` 단건 조회 화면은 공개 접근 허용
...
- Integration: CRUD API, 인증 기반 타임라인 접근 제어, 단건 공개 조회, S3 presigned URL 발급
