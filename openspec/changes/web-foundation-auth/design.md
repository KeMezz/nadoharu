...
- 선택: `apps/web`에서 인증 요청 진입점을 단일화한다(환경별 URL 분기 최소화, 쿠키 전달 규칙 일관화).
...
- 선택: 미들웨어/라우팅 단계에서 1차 접근 제어를 수행하고, 필요 시 `me` 조회로 인증 상태를 확정한다.
...
### 9) 소셜 로그인 버튼은 외관만 제공하고 비활성 상태로 유지
