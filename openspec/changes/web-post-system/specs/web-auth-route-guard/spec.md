...
시스템은 `/me` 및 후속 탭 영역 라우트를 보호 라우트로 처리해야 하며 비인증 접근을 차단해야 한다(MUST). 시스템은 `/posts`를 비인증 접근 허용 라우트로 처리하되 비인증 사용자에게 로그인 안내 UX를 표시해야 한다(MUST). 시스템은 `/posts/[id]`를 공개 라우트로 처리해야 하며 비인증 접근 시 로그인 리다이렉트를 수행하지 않아야 한다(MUST).
