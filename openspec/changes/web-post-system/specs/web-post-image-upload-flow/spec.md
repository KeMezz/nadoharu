...
시스템은 이미지 업로드 허용 포맷 및 최대 용량 정책을 공유 모듈(`packages/shared`)에서 단일 소스로 관리해야 한다(MUST). 프론트엔드는 해당 정책으로 사전 검증을 수행해야 하며(MUST), API는 클라이언트 검증 여부와 무관하게 동일 정책을 최종 강제해야 한다(MUST).
...
#### Scenario: 우회 요청에 대한 API 최종 차단

- **WHEN** 클라이언트 우회로 정책 위반 이미지 메타데이터가 API에 전달된다
- **THEN** API는 요청을 거부하고 presigned URL을 발급하지 않는다
