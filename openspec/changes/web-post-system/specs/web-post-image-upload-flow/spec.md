...
#### Scenario: presigned URL 발급 단계 실패

- **WHEN** 사용자가 업로드를 시작했지만 presigned URL 발급 API 호출이 인증 만료, 검증 오류, 또는 서버 오류로 실패한다
- **THEN** 시스템은 스토리지 업로드를 시도하지 않고 실패 원인과 재시도 동작을 표시한다
