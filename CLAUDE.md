...
- `apps/web` UI/UX 구현 시 레거시 `nadoharu-front` 디자인을 우선 참조한다(MUST).
- 레거시 참조 코드는 루트 `.legacy/nadoharu-front` 경로를 사용한다(MUST).
- `.legacy` 디렉터리는 로컬 참조 전용이며 Git 추적 대상에서 제외한다(MUST). 단, 운영 규칙 문서인 `.legacy/README.md`는 예외로 추적한다.
- 레거시 코드는 디자인/인터랙션 참고용으로만 사용하고, 데이터 계약/GraphQL/라우팅/상태 관리는 현재 모노레포 기준으로 재작성한다(MUST).
- PR/스펙/문서에 로컬 절대 경로(예: `/home/...`)를 기록하면 안 된다(MUST NOT). 레퍼런스는 항상 `nadoharu-front`와 같은 일반 명칭으로 표기한다.
